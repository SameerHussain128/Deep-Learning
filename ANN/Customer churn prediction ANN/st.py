...
sc = joblib.load('scaler.pkl')  # Assuming you also need to load the scaler
...
tenure = st.number_input('Tenure (in years)', min_value=0, max_value=50, value=5)
...
def preprocess_input(data):
    ...
    # Apply StandardScaler (assuming `sc` is the loaded scaler)
    df_scaled = sc.transform(df)[:, :-1]  # Remove the last dummy column (Gender_Male) as it's not used in training
